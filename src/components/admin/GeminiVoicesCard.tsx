...
export function GeminiVoicesCard() {
  const queryClient = useQueryClient();
  const { user } = useAuthContext();
  const { data: allVoices, isLoading } = useGeminiVoicesAdmin();
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewName('');
      setNewLabel('');
      toast.success('Gemini voice added successfully');
    },
  ...
  const startEdit = (voice: GeminiVoice) => {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Manage Google Gemini voices. Uses voice name directly (no Voice ID needed).
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        {showAdd && (
          <div className="border rounded-lg p-4 space-y-3 bg-muted/30">
            <div className="grid grid-cols-2 gap-3">
              <Input
                placeholder="Voice name (e.g. Kore, Puck)"
                value={newName}
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-8">
            No Gemini voices found. Add one to get started.
          </p>
        ) : (
